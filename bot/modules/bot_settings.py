...
def load_config():
    ...
    if len(BASE_URL) == 0:
        ...
        srun(["pkill", "-9", "-f", "gunicorn"])
        Popen(f"gunicorn web.wserver:app --workers 1 --threads 1 --timeout 86400 --bind 0.0.0.0:{SERVER_PORT}", shell=True)
...
def edit_variable(update, context, omsg, key):
    ...
    if value.lower() == 'true':
        ...
        value = int(value)
        srun(["pkill", "-9", "-f", "gunicorn"])
        Popen(f"gunicorn web.wserver:app --workers 1 --threads 1 --timeout 86400 --bind 0.0.0.0:{value}", shell=True)
...
def edit_bot_settings(update, context):
    ...
    if not CustomFilters.owner_query(user_id):
        ...
        if data[2] in default_values:
            ...
            value = 80
            srun(["pkill", "-9", "-f", "gunicorn"])
            Popen("gunicorn web.wserver:app --workers 1 --threads 1 --timeout 86400 --bind 0.0.0.0:80", shell=True)
